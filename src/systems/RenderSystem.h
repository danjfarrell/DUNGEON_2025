...
private:
    ...
    TileVisibility* tile_visibility;  // ADD THIS LIN

public:
    RenderSystem(SpriteManager* sm, int scale = 2, Camera* cam = nullptr, UILayout* layout = nullptr, TileVisibility* visibility = nullptr)
        : sprite_manager(sm), tile_scale(scale), camera(cam), ui_layout(layout), tile_visibility(visibility) {
    }
