...
public:
    Camera(int screen_w, int screen_h, int map_w, int map_h, int tile_sz)
        : x(0), y(0),
    ...
    void center_on(int tile_x, int tile_y) {
        ...
        x = pixel_x - screen_width / 2;
        y = pixel_y - screen_height / 2;
    ...
    void smooth_follow(int tile_x, int tile_y, float lerp_factor = 0.1f) {
        int target_x = tile_x * tile_size - screen_width / 2;
        int target_y = tile_y * tile_size - screen_height / 2;
    ...
    bool is_tile_visible(int tile_x, int tile_y) const {
        ...
        return pixel_x + tile_size >= x &&
            pixel_x < x + screen_width &&
            pixel_y + tile_size >= y &&
            pixel_y < y + screen_height;
    }
